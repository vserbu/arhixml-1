...
public class MediumCustomFieldController implements Handler, ItemClickListener {
    private static final long serialVersionUID = 1L;
    private final Logger logger = LoggerFactory.getLogger(MediumCustomFieldController.class.getName());
    ...
    public void handleAction(Action action, Object sender, Object target) {
        ...
        if (action == actionRemove) {
            mediumsCustomField.removeSelectedItems();
        }

        if (action == actionAdd) {
            mediumsCustomField.addNewItem();
        }
     ...
     * item from the table which holds {@link Medium} objects.
     * <p>
    ...
    public void itemClick(ItemClickEvent event) {
        ...
        if (event.getButton() == ItemClickEvent.BUTTON_RIGHT) {
            mediumsCustomField.selectTableItem(event.getItemId());
        }
