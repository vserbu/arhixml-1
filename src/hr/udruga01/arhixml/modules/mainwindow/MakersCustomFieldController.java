...
public class MakersCustomFieldController implements Handler, ItemClickListener {
    private static final long serialVersionUID = 1L;
    private final Logger logger = LoggerFactory.getLogger(MakersCustomFieldController.class.getName());
    ...
    public void handleAction(Action action, Object sender, Object target) {
        ...
        if (action == actionRemove) {
            makersCustomField.removeSelectedItems();
        }

        if (action == actionAdd) {
            makersCustomField.addNewItem();
        }
     ...
     * item from the table which holds {@link Maker} objects.
     * <p>
    ...
    public void itemClick(ItemClickEvent event) {
        ...
        if (event.getButton() == ItemClickEvent.BUTTON_RIGHT) {
            makersCustomField.selectTableItem(event.getItemId());
        }
