...
public class ArchiveUnitsCustomFieldController implements Handler, ItemClickListener {
    private static final long serialVersionUID = 1L;
    private final Logger logger = LoggerFactory.getLogger(ArchiveUnitsCustomFieldController.class.getName());
    ...
    public void handleAction(Action action, Object sender, Object target) {
        ...
        if (action == actionRemove) {
            archiveUnitsCustomField.removeSelectedItems();
        }

        if (action == actionAdd) {
            archiveUnitsCustomField.addNewItem();
        }
     ...
     * item from the table which holds {@link ArchiveUnit} objects.
     * <p>
    ...
    public void itemClick(ItemClickEvent event) {
        ...
        if (event.getButton() == ItemClickEvent.BUTTON_RIGHT) {
            archiveUnitsCustomField.selectTableItem(event.getItemId());
        }
